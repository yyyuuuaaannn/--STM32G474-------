...
//	__HAL_HRTIM_SETCOMPARE(&hhrtim1, HRTIM_TIMERINDEX_TIMER_A, HRTIM_COMPAREUNIT_1, (uint16_t)(54400*0.8));
//	HAL_HRTIM_WaveformOutputStart(&hhrtim1, HRTIM_OUTPUT_TA1 | HRTIM_OUTPUT_TA2);
  ...
  while (1)
  {
		...
		if(!OLED_counter)
		{
////			arm_mean_f32(Id_buffer, 50, &Id_mean);
////			arm_mean_f32(Iq_buffer, 50, &Iq_mean);
////			arm_sqrt_f32(Id_mean*Id_mean + Iq_mean*Iq_mean, &pf);
//			pf_counter++;
//			if(pf_counter>=10)
//				pf_counter=0;
//			arm_sqrt_f32(Id*Id + Iq*Iq, &pf);
//			pf = Id / pf;
//			pf_buffer[pf_counter] = pf;
//			arm_min_f32(pf_buffer, 10, &pf, &pf_index);
//			arm_mean_f32(pf_buffer, 10, &pf);
//			pf -= 0.01;
			OLED_Printf(0, 0, OLED_6X8, "PF:%.3f", pf);
			OLED_Printf(0, 8, OLED_6X8, "W:%.3f", phase_W);
//			OLED_Printf(0, 8, OLED_6X8, "Iq:%.3f", Iq_mean);
//			OLED_Printf(0, 16, OLED_6X8, "Id:%.3f", Id_mean);
			OLED_Update();
			OLED_counter = 400;
		}
		else
...
//		LIMIT(out_pwm,2720,48960); 
		...
		if(!SOGI_I_counter)
		{
			...
			out_U = 0.02362 * ADC1_Buffer[1] - 48.24810356169533;		//A=0.02368198477586693
...
//		Serial_TX_Buffer.TX_Data[0] = sogi_s.Ua;
//		Serial_TX_Buffer.TX_Data[1] = sogi_s.Ub;
//		Serial_TX_Buffer.TX_Data[2] = d;
//		Serial_TX_Buffer.TX_Data[3] = q;
		
//		Serial_TX_Buffer.TX_Data[0] = (float32_t)ADC1_Buffer[0];
//		Serial_TX_Buffer.TX_Data[1] = (float32_t)ADC1_Buffer[1];
//		Serial_TX_Buffer.TX_Data[2] = (float32_t)ADC2_Buffer[0];
//		Serial_TX_Buffer.TX_Data[3] = (float32_t)ADC2_Buffer[1];
		
//		Serial_TX_Buffer.TX_Data[0] = w / 2 / PI;
//		Serial_TX_Buffer.TX_Data[1] = out_duty;
//		Serial_TX_Buffer.TX_Data[2] = sogi_s.Ug;
//		Serial_TX_Buffer.TX_Data[3] = q;
		
//		Serial_TX_Buffer.TX_Data[0] = out_duty;
//		Serial_TX_Buffer.TX_Data[1] = sogi_s_I.Ug;
//		Serial_TX_Buffer.TX_Data[2] = sogi_s.Ug;
//		Serial_TX_Buffer.TX_Data[3] = out_U;

		Serial_TX_Buffer.TX_Data[0] = out_duty;
		Serial_TX_Buffer.TX_Data[1] = out_amplitude;
		Serial_TX_Buffer.TX_Data[2] = ADC1_Buffer[1];
		Serial_TX_Buffer.TX_Data[3] = out_U;
